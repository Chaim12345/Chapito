...
async def send_message(page, message: str) -> bool:
    ...
    try:
        ...
        textareas = await page.find(by=By.CLASS_NAME, value="textarea", find_all=True)
        if not textareas:
...
async def chat_with_gemini(page, message: str) -> str:
...
async def main():
    ...
    try:
        ...
        if not await navigate_to(page, URL):
            logging.error("Failed to navigate to Gemini")
            return
        ...
        response = await chat_with_gemini(page, message)
