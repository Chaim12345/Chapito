...
async def send_message(page, message: str) -> bool:
    ...
    try:
        ...
        textareas = await page.find(by=By.TAG_NAME, value="textarea", find_all=True)
        if not textareas:
            logging.error("No textarea found")
            return False
        
        textarea = textareas[-1]  # Use the last textarea
...
async def chat_with_ai_studio(page, message: str) -> str:
    """Main function to chat with AI Studio."""
    try:
        # Send the message
        ...
        logging.error(f"Error in chat_with_ai_studio: {e}")
...
async def main():
    ...
    try:
        ...
        if not await navigate_to(page, URL):
            logging.error("Failed to navigate to AI Studio")
            return
        ...
        response = await chat_with_ai_studio(page, message)
        print(f"AI Studio Response: {response}")
