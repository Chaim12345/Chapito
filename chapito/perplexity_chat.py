...
async def chat_with_perplexity(page, message: str) -> str:
    """Main function to chat with Perplexity."""
    try:
        # Send the message
        ...
        logging.error(f"Error in chat_with_perplexity: {e}")
...
async def main():
    ...
    try:
        ...
        if not await navigate_to(page, PERPLEXITY_URL):
            logging.error("Failed to navigate to Perplexity")
            return
        ...
        response = await chat_with_perplexity(page, message)
