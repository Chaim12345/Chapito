...
async def send_message(page, message: str) -> bool:
    ...
    try:
        ...
        textarea = await page.find(by=By.XPATH, value=QUESTION_XPATH)
        if not textarea:
...
async def chat_with_kimi(page, message: str) -> str:
    """Main function to chat with Kimi."""
    try:
        # Send the message
        ...
        logging.error(f"Error in chat_with_kimi: {e}")
...
async def main():
    ...
    try:
        ...
        if not await navigate_to(page, URL):
            logging.error("Failed to navigate to Kimi")
            return
        ...
        response = await chat_with_kimi(page, message)
